...
class CartFSMTest extends TestKit(ActorSystem("CheckoutTest"))
  with FlatSpecLike
  with ImplicitSender
...
object CartFSMTest {
  ...
  def cartActorWithCartSizeResponseOnStateChange(system: ActorSystem) = system.actorOf(Props(new CartFSM {
